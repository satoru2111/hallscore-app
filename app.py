...
def hall_result_card(hall: str, nodata: bool) -> None:
    ...
    with st.container(border=True, key=f"res_{_ci()}"):
        ...
        if not po.empty and pd.notna(po["mai"].iloc[0]):
            bits.append(f"普段の出方 **{po['mai'].iloc[0]:+,.0f}枚/台**（打たれた1ゲームあたりの出玉率を1台1日 4,800回転で換算）")
...
def hall_traits() -> dict:
    ...
    # 日付末尾に乗る店 (2026-09-06, #89/#96)。台番末尾 = その日の日付末尾 の台が同じ店日の他の台より
    # 甘いかを店ごとに検定した結果 (build_tail_date_stats.py)。level は検定で付く (閾値の目安ではない)。
    # 「高」= レポート20以上・効果>0・符号反転 p<0.05。3割の日に日付末尾へ設定6級を置く店なら 79% で拾う
    try:
        tdd = q("""SELECT hall, level, eff_mean, n_reports FROM hall_tail_date_stats WHERE level IS NOT NULL""")
        for hall, lv, eff, n in tdd.itertuples(index=False):
            put(hall, "日付末尾", lv, f"日付末尾の台が他より出玉率 {eff:+.1f}pt (約{eff*144:+,.0f}枚/日、{int(n)}店日)")
    except Exception:
        pass    # serve.db が古く表が無い間は出さない
    # 角
...
def trait_label(name: str) -> str:
    ...
    if name.startswith("末尾"):
        return f"{name}が出ていた"
    return {"角(並びの端)": "端が出ていた", "連続投入": "隣同士で出ていた",
            "日付末尾": "日付末尾に乗る店"}.get(name, name)
...
def trend_qa() -> list[dict]:
    ...
    # 2026-09-06 (#89) で作り直し。旧 a8「同じ末尾に設定6級を3台、515店中ゼロ」は「毎日やる癖」しか
    # 見ない設計で、日付に紐づく単発 (7日→末尾7 は月に3日) を否定できていなかった。
    # v2: 台番末尾 = その日の日付末尾 の台 vs 同じ店日の他の台 (機種調整済み出玉率)。
    # イベント日 25,674 店日で 台日平均 +0.23pt / 全台が回された島日 +0.38pt (z 6.3、前後半 +0.39/+0.37)。
    # 全店毎日に設定6級を入れると +9.8pt → 頻度 ≈ 4% の店日。店別: 甘い側で有意 41〜42 店 vs 期待 10.6。
    # 店別の結果は hall_tail_date_stats → 店ごとのバッジ「日付末尾に乗る店」。
    n_hi = con.execute("SELECT COUNT(*) FROM hall_tail_date_stats WHERE level='高'").fetchone()[0] \
        if con.execute("SELECT name FROM sqlite_master WHERE name='hall_tail_date_stats'").fetchone() else 0
    qa.append({"q": "末尾(台番の下1桁)は関係ある?",
               "a": "日付の末尾と同じ末尾の台は、イベント日に少しだけ甘い。店日の約4%、店の1割弱の癖として実在します",
               "d": "2026年9月に調べ直しました。以前は「末尾に固める店は無い」と書いていましたが、それは"
                    "「毎日同じ末尾に入れる店」を探す方法で、日付に紐づく単発（7日に末尾7、など）を見落とす設計でした。"
                    "イベント日 25,674 店日で「台番の末尾がその日の日付の末尾と同じ台」を同じ店日の他の台と比べると、"
                    "機種の違いを揃えた出玉率で +0.23pt、島の全台が回された日に絞ると +0.38pt 甘い"
                    "（1台1日 +35〜55枚。前半後半で同じ）。全店が毎日やっていれば +9.8pt になる計算なので、"
                    "実際に乗っているのはイベント日の店日の 4% ほどです。店ごとに検定すると、日付末尾が甘い側で"
                    f"有意な店が {n_hi} 店（偶然なら 11 店）。該当する店には「日付末尾に乗る店」のバッジを付け、"
                    "店舗情報に数字を出しています。注意: この方法は「3割の日に日付末尾へ設定6級を置く店」を 79% で"
                    "見つけますが、1割以下の頻度の店は見つけられません。バッジが無い店に癖が無いとは言えません。"
                    f"参考: 全店平均の末尾差は {tail:+,.0f}枚程度で、末尾それ自体には差がありません。"})
    # 据え置き (2026-09-06, #89 C): 連続日ペア 422,446 で台別残差の相関 −0.0137 (z −16.7)、
    # 両日とも全台 >=2000G でも −0.0073 (z −5.9)。3台に設定6級を両日入れる注入で +0.067 になる統計量。
    # 読み手の問い B〜H (2026-09-06, #101/#107)。数字は 2026-09-06 時点。指標に依らない結論だけ
    qa.append({"q": "凹んだ台は翌日出る? 出た台は翌日回収?",
               "a": "凹み台は翌日も普通です。出た台は翌日少しだけ下がります",
               "d": "同じ店・同じ機種の連続する2日を約21万組集め、前日に下位1割だった台と上位1割だった台の翌日を、"
                    "中位の台と比べました（両日ともよく回された島だけで）。凹み台の翌日は +0.15pt で誤差の範囲、"
                    "出た台の翌日は −0.13〜−0.19pt（1台1日 −20〜−27枚、誤差の1.4〜2倍）。"
                    "「凹んでいるから明日は出る」に根拠はなく、「出た台は翌日回収」は小さいながら向きは合っています。"})
    qa.append({"q": "イベントの翌日は回収日?",
               "a": "そうは見えません。翌日も前日も、ふつうの日とほぼ同じです",
               "d": "569店で「イベント翌日の非告知日」の出玉率をその店の他の非告知日と比べると +0.13pt。"
                    "同じ計算を「イベント前日」でやっても +0.12pt で、前後で対称です。翌日に締める傾向は出ませんでした。"})
    qa.append({"q": "1日・7の付く日・給料日後・月末・ゾロ目は出る?",
               "a": "どれも差は +0.3pt 以下。日付で店を選ぶ意味は小さい",
               "d": "非告知日について、店ごとに日付カテゴリの出玉率をその店の平均と比べました（打たれた1ゲームあたりの出玉率）。"
                    "1日 +0.30 / 7の付く日 +0.17 / 給料日後(25〜27日) +0.11 / 月末 +0.13 / ゾロ目(11・22日) +0.04pt。"
                    "全部わずかにプラスで、1台1日 +40枚以下です。イベント日の日付末尾（末尾の項）とは別の話です。"})
    qa.append({"q": "系列（チェーン）で店を選んでいい?",
               "a": "系列の差より、同じ系列の中の店の差のほうが大きい",
               "d": "店名の先頭語で系列にまとめると（8店以上の系列 7〜11）、系列で説明できる甘辛の差は全体の 10〜13% だけ。"
                    "系列の平均差は ±1pt 以内で、同じ系列の店どうしのばらつき（0.6〜1.5pt）のほうが大きい。"
                    "県と同じで、系列は店選びの代わりになりません。"})
    qa.append({"q": "7の付く台番は出る?",
               "a": "末尾77 だけ少し（+0.23pt）。他はほぼ差なし",
               "d": "イベント日のレポート内で、台番に 7 を含む台 +0.05pt、末尾7 +0.07pt、末尾77 +0.23pt（1台1日 +33枚、誤差の2.7倍）。"
                    "非告知日はどれも差なし。末尾77 は店が意識して入れている可能性がありますが、額は小さい。"
                    "777 番の台はデータにありませんでした。"})
    qa.append({"q": "このサイトの数字は甘めに出ていない?",
               "a": "出ている可能性があります。投稿された日は甘い側に選ばれやすい",
               "d": "同じ店で「全日一覧から集めた日」と「イベント投稿と同じ経路で拾った非告知日」を比べると、"
                    "前者が +1.5〜1.8pt 甘い（3つの指標で一致）。人は出た日のデータを投稿しやすいので、"
                    "投稿に依存する数字は水準が上に寄ります。店どうしの比較（順位）には同じ向きで乗りますが、"
                    "「この店は平均で +N枚」という水準は控えめに読んでください。全日一覧の収集を広げて是正中です。"})
    qa.append({"q": "昨日出た台は据え置きで今日も出る?",
               "a": "逆です。前日にその島で出た台は、翌日その島の平均より下がります",
               "d": "同じ店・同じ機種の連続する2日を 42万組集め、台ごとの出玉率（その日の機種平均からのずれ）が"
                    "前日と翌日でどれだけ一致するかを見ました。一致しないどころか、わずかに逆向き"
                    "（相関 −0.014。両日とも全台がよく回された日に絞っても −0.007 で、偶然の 0 より有意に負）。"
                    "前日に出た台を翌日に追いかけると、平均では損をします。店が出た台を動かしている"
                    "（回収している）と読むのが自然です。据え置きをする店が一部にあっても、全体では打ち消されて"
                    "見えない水準です。"})
    # 角台
    ...
    # 2026-09-06 (Issue #81/#83) で「逆です。新台ほど辛い」を撤回。生の差 (差枚 −457枚、出率 −0.91pt) は
    # 事実だが、(1) 新台は別機種で spec が違う (2) 負けても粘られるので games 加重が下に偏る、の2つで
    # 大半が説明できた。同一店日 × 機種調整で −0.44 / +0.55pt (加重 / 台日平均) と 0 を挟む。
    # 「新台に高設定を入れる店」が前後半で再現しなかった (r=+0.073) 点は変わらない。
    # 空き台の選ばれ方: 早く捨てられた新台は機種日平均より −15.5pt (捨てられた定着より −3.0pt)、
    # ただし新台の早捨ては 0.6% と稀 (定着 6.2%)。X には訂正投稿済み。
    qa.append({"q": "新台は甘い?", "a": "出るとも出ないとも言えません。ただし空いている新台は諦められた台です",
               "d": f"単純な平均では導入0-14日 {round(new[0]):+,.0f}枚 → 121日以上 "
                    f"{round(new[1]):+,.0f}枚と新台のほうが悪く見えます。以前はこれを根拠に"
                    "「新台ほど辛い」と書いていましたが、2026年9月に調べ直して撤回しました。"
                    "差の大半は「新台は別の機種で、機種ごとに出玉率の水準が違う」ことと、"
                    "「新台は負けていても長く打たれる（大負けの台で定着機種の1.19倍回されている）ので、"
                    "回転数で重みづけた平均が下に偏る」ことで説明できます。同じ店・同じ日で機種の違いを"
                    "揃えて比べると、新台と定着機種の差は ±0.5pt（1台1日 ±70枚）の誤差の範囲で、"
                    "「新台に設定が入る」と言えるほどの差（+5pt 以上）はありません。"
                    "代わりに分かったのは席の選ばれ方です。新台で早く捨てられた台は、その機種のその日の平均より "
                    "15pt 低く、捨てられた定着機種より 3pt 悪い。新台は基本的に埋まっていて（早く捨てられるのは "
                    "0.6%、定着機種は 6.2%）、空きが出るのは粘った人でさえ諦めたときです。"
                    "「新台に高設定を入れる店」も探しましたが、前半のデータで見つけた店は後半で再現しませんでした。"})
...
# ---------- ② ジャグラーランキング ----------
    ...
    with st.expander(_lab, expanded=False):
        ...
        if metric == "ジャグラーが甘い順":
            ...
            # 2026-09-06 (#104): 値は「回転数加重の出玉率」を 4,800 回転で枚に換算したもの。
            # #99 の「2,000G 以上の台日平均」は混む店ほど辛く見える選択の偏り (稼働と −0.70) で撤回。
            # 全台の台日平均は逆向き (+0.78)。加重は最も中立 (+0.18) (#101 F/H)
            st.caption("枚数は「その店で打たれた1ゲームあたりの出玉率」を1台1日 4,800回転で換算した値です。回転数の多い店ほど大きく出る偏りは入りません。"
                       "投稿されたデータは出た日に偏りやすいので、水準は控えめに読んでください（順位の比較には影響しにくい）。")
...
# 店舗情報の内部に st.stop() があるため、そこでスクリプトが止まる (2026-09-01 修正)
    ...
    def _trait_detail(name: str):
        ...
        if name == "日付末尾":
            d = q("""SELECT n_reports AS "イベント日数", eff_mean AS "日付末尾の台 − 他 (pt)",
                            eff_full AS "全台が回された日だけ (pt)", n_full AS "その日数", p_sign AS "偶然で出る確率"
                     FROM hall_tail_date_stats WHERE hall=?""", (hsel,))
            return d
        if name.startswith("末尾"):
...
if page == "傾向":
    ...
    if _sub == "検証":
        ...
        # ---------- 公開トラックレコード (#102, 2026-09-06) ----------
        # 毎晩「サイトが見せた狙い目」を凍結し、翌日以降の実績で答え合わせ。外れた週も全部載せる。
        ...
        with st.container(border=True, key=f"card_tr_{_ci()}"):
            ...
            if trw.empty:
                ...
                lab = {"both": "検証済みの狙い目", "x_daily": "X の日次投稿"}
                show = trw.copy()
                show["予想"] = show["tier"].map(lab)
                show["的中率"] = show["hit_rate"].map(lambda v: f"{100*v:.0f}%" if pd.notna(v) else "—")
                show["基準の的中率"] = show["base_hit_rate"].map(lambda v: f"{100*v:.0f}%" if pd.notna(v) else "—")
                show["実現 (枚/台日)"] = show["mean_mai"].map(lambda v: f"{v:+,.0f}" if pd.notna(v) else "—")
                show["基準 (枚/台日)"] = show["base_pt"].map(lambda v: f"{v/100*3*4800:+,.0f}" if pd.notna(v) else "—")
                st.dataframe(show.rename(columns={"week_start": "週", "n_pred": "予想数", "n_real": "確認できた数", "n_no_report": "レポート無し"})
                             [["週", "予想", "予想数", "確認できた数", "レポート無し", "的中率", "実現 (枚/台日)", "基準の的中率", "基準 (枚/台日)"]],
                             hide_index=True, width="stretch")
                r8 = trw[trw["tier"] == "both"].head(8)
                if not r8.empty and r8["n_real"].sum() > 0:
                    w = r8["n_real"].fillna(0)
                    hit = (r8["hit_rate"].fillna(0) * w).sum() / w.sum()
                    bw = r8["base_n"].fillna(0); bhit = (r8["base_hit_rate"].fillna(0) * bw).sum() / max(bw.sum(), 1)
                    st.caption(f"直近 {len(r8)} 週の合計: 検証済みの狙い目 的中率 {100*hit:.0f}%（基準 {100*bhit:.0f}%）、"
                               f"確認できた予想 {int(w.sum())} 件。判定線: 8 週の的中率が基準を上回り、実現の平均が 0 より大きいこと。"
                               "下回り続ければ狙い目の表示を止めます。")
                st.caption("的中 = 予想した店のその日の出玉率（打たれた1ゲームあたり）が、その店の通常日を上回ったこと。"
                           "基準 = 同じ日にデータがあり予想に入っていなかった店。「レポート無し」は答え合わせできなかった件数で、隠していません。")
